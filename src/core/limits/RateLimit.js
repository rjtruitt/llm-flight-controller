...
class RateLimit {
    constructor(config) {
        ...
        this.tpmWindow = new RollingWindow_1.RollingWindow({ windowMs: 60000 }); // 1 minute
        this.rpmWindow = new RollingWindow_1.RollingWindow({ windowMs: 60000 });
        this.tphWindow = new RollingWindow_1.RollingWindow({ windowMs: 3600000 }); // 1 hour
        this.rphWindow = new RollingWindow_1.RollingWindow({ windowMs: 3600000 });
    }
    /**
    ...
    checkLimit(usage) {
        ...
        if (currentTPM + tokens > this.config.tpm) {
            ...
            const waitMs = oldestEntry ? oldestEntry.timestamp + 60000 - now : 60000;
            return {
                allowed: false,
                reason: 'tpm_exceeded',
     ...
     */
    isApproachingLimit(now = Date.now()) {
